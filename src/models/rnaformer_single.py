...
class RNAformerS(nn.Module):
    ...
    def forward(self, input_sequence, codon_adaptation_index=CAI_TEMPLATE, mask=None):
        ...
        # print(codon_adaptation_index.shape)
    
        codon_adaptation_index_memory = codon_adaptation_index.repeat(
            1, amino_acid_positioned.size(1), int((self.hidden_dim) / codon_adaptation_index.size(2))
        ) 
        # amino_acid_positioned = torch.cat(
        #     (amino_acid_positioned , XXXX ), dim=2
        # )
